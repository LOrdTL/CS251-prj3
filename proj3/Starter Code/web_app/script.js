...
async function add_IOU(creditor, amount) {
    ...
    let cycle = await doBFS(creditor, defaultAccount, getNeighbors);

    if (cycle) {
        console.log("Debt cycle detected:", cycle);
        await simplifyDebtCycle(cycle);
    } else {
        let tx = await contractWithSigner.add_IOU(creditor, amount);
        await tx.wait();
        console.log(`IOU added: ${creditor} is owed ${amount}`);
    }
}
async function simplifyDebtCycle(cycle) {
    let minDebt = Infinity;
    for (let i = 0; i < cycle.length - 1; i++) {
        let debtor = cycle[i];
        let creditor = cycle[i + 1];
        let amountOwed = await BlockchainSplitwise.lookup(debtor, creditor);
        minDebt = Math.min(minDebt, parseInt(amountOwed));
    }

    console.log("Min debt in cycle:", minDebt);
    for (let i = 0; i < cycle.length - 1; i++) {
        let debtor = cycle[i];
        let creditor = cycle[i + 1];
        ...
        let tx = await contractWithSigner.add_IOU(creditor, -minDebt);
        await tx.wait();
    }
